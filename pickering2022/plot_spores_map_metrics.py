...
def make_plot(
    path_to_friendly_data, path_to_units, path_to_unit_groups, spore_num, path_to_output
):
    """

    :param path_to_friendly_data:   path to the dataset in friendly data format (should contain 'datapackage.json')
    :param path_to_units:           path to .geojson file (units are the regions defined in 'eurospores_units.geojson')
    :param path_to_unit_groups:     path to .csv file that defines unit groups (units groups are geographically grouped regions defined in 'plotting_unit_groups.csv', unit groups are used for plotting Figure 5A and 5D in the paper)
    :param spore_num:               SPORES number to plot figure
    :param path_to_output:          path to output folder where figuer is saved
    :return:
    """
    ...
    unit_groups = (
        pd.read_csv(path_to_unit_groups, index_col=0).squeeze(1).reindex(units.index)
    )
    ...
    """
    This loop reads all data in the friendly_data folder defined by datapackage.json
        --> resource contains a dict for each .csv file in the data
    """
    for resource in spores_datapackage["resources"]:
        print("reading: " + resource["path"])
...
def get_output_energy(spores_data, spores=None):
    ...
    flow_out_summed = (
        flow_out.unstack("spore")
        .groupby(level=["techs", "carriers"])
        .sum()
        .groupby(ENERGY_PRODUCERS, level="techs")
        .sum()
    )
    flow_out_summed.loc["Nuclear heat"] *= NUCLEAR_HEAT_MUTIPLIER

    flow_out_summed.loc[:, "min"] = flow_out_summed.min(axis=1)
    flow_out_summed.loc[:, "max"] = flow_out_summed.max(axis=1)

    if spores is None:
        print("TAG: spores == none")
        return flow_out_summed
    else:
        print("TAG: spores =! none")
        return flow_out_summed[spores].loc[
            flow_out_summed[spores].mean(axis=1).sort_values(ascending=False).index
        ]
...
def get_pv_wind_grouped_region_prod(result_dict, units_grouped, spore_names):
    prod = (
        ...
        .unstack("spore")
        .groupby(
            [ENERGY_PRODUCERS, units_grouped.Group.to_dict()], level=["techs", "locs"]
        )
        .sum()
    ...
    prod.loc[:, "min"] = prod.min(axis=1)
    prod.loc[:, "max"] = prod.max(axis=1)
...
def get_region_net_import(result_dict):
    net_import = result_dict["net_import_sum"].unstack("spore")
    net_import = (
        net_import[
            net_import.index.get_level_values("importing_region")
            != net_import.index.get_level_values("exporting_region")
        ]
        .groupby(level="importing_region")
        .sum()
    )
    import_cbar_max = net_import.abs().max().max()
    net_import.loc[:, "min"] = net_import.min(axis=1)
    net_import.loc[:, "max"] = net_import.max(axis=1)
...
def get_synfuel_contribution(result_dict):
    ...
    synfuel_contribution.loc[:, "min"] = synfuel_contribution.min(axis=1)
    synfuel_contribution.loc[:, "max"] = synfuel_contribution.max(axis=1)
...
def get_link_caps(result_dict):
    ...
    link_caps.loc[:, "min"] = link_caps.min(axis=1)
    link_caps.loc[:, "max"] = link_caps.max(axis=1)
...
def plot_comparison_maps(
    ...
    with sns.plotting_context("paper", font_scale=1.5):
        ...
        for spore_num, spore_name in spore_name_dict.items():
            ...
            alpha_idx += 1
            ax[spore_num]["import_synfuel_map"] = plt.subplot(
                gs[_row + 1, 3], frameon=False
            )
            cmap = mpl.cm.coolwarm(np.linspace(0, 1, 200))
            import_cmap = mpl.colors.ListedColormap(cmap)
            synfuel_cmap = mpl.colors.LinearSegmentedColormap.from_list(
                "", ["yellow", "grey", "black"]
            )
            ...
            alpha_idx += 1
            ax[spore_num]["transmission_expansion_map"] = plt.subplot(
                gs[_row + 1, 4], frameon=False
            )
        ...
        ax[spore_num]["primary_energy_legend"] = plt.subplot(
            gs[-2:, 0:3], frameon=False
        )
        plot_regional_and_total_primary_energy_supply_legend(
            ax[spore_num]["primary_energy_legend"], handles, labels, ncol=3
        )
        ...
        plot_transmission_expansion_legend(
            ax[spore_num]["transmission_legend"], link_cap_mean, link_cap_max
        )
...
def plot_regional_energy_supply(
    ...
    def _build_bar(mapx, mapy, ax, width, hideaxis=True):
        ax_h = ax.inset_axes(
            [mapx - width / 2, mapy - width / 2, width, width],
    ...
    for group in group_data.index:
        if group_data.loc[group].sum() > prod_regional_ymax * 0.05:
            ...
            ax_h.set_xlabel("")
            ax_h.set_ylabel("")
            grouped_units_m.loc[[group]].plot(
                ax=ax_map, fc="#E0ECF8", ec="white", lw=0.5
            )
            ...
            grouped_units_m.loc[[group]].plot(
                ax=ax_map, fc="lightgrey", ec="white", lw=0.5
            )
    units_m.plot(ax=ax_map, fc="None", linestyle="-", ec="white", lw=0.1)
    print(
        f"{100 * _selected_data / group_data.sum().sum():.2f}% Production plotted for SPORE {spore_num}"
    )
...
def plot_total_energy_supply(
    ax_total_bar, prod_sum, prod_sum_max, spore_num, spore_name, tech_colors
):
...
def plot_regional_and_total_primary_energy_supply_legend(
    ax_legend, handles, labels, ncol=4
):
    ax_legend.axis("off")
    ax_legend.legend(
        handles=handles, labels=labels, ncol=ncol, frameon=False, loc="center"
    )
...
def plot_imports_and_synfuel_overprod_legend(
    ...
    sm = plt.cm.ScalarMappable(
        cmap=import_cmap,
        norm=plt.Normalize(vmin=-import_cbar_max / 10000, vmax=import_cbar_max / 10000),
    )
    cbar = plt.colorbar(
        sm,
        ax=ax_import_legend,
    ...
    sm = plt.cm.ScalarMappable(
        cmap=synfuel_cmap, norm=plt.Normalize(vmin=0.05, vmax=overprod_cbar_max)
    )
    cbar = plt.colorbar(
        sm,
        ax=ax_synfuel_legend,
        orientation="horizontal",
        pad=0.1,
        anchor=(0.5, 1.5),
        aspect=40,
...
def plot_transmission_expansion_legend(ax_legend, link_cap_mean, link_cap_max, ncol=1):
    existing = mpl.lines.Line2D(
        xdata=[0, 1], ydata=[0, 1], color="#a9a9a999", lw=0.5, label="Existing link"
    )
...
def plot_title(ax_title, spore_name):
    ax_title.axis("off")
    ax_title.text(0, 0.5, spore_name, va="bottom", ha="left", fontweight="bold")


if __name__ == "__main__":
    """
    Testplot of geojson file
    """
    ...
    """
    Plot geographical figure that can be seen on explore.callio.pe
    """
    make_plot(
        ...
        spore_num=1,
        path_to_output="../figures/spores_map.png",
    )
